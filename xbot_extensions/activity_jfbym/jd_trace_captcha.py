...
def recg(token, base64_im):
    ...
    data = { 'image': base64_im, 'token': token, 'type': 100015 }
...
def yd_recg(image_base64):
    ...
    payload = json.dumps({
        "thirdPartyInterfaceCode": "jfbym_100015",
        "data": {},
        "thirdPartyReqData": {
            "headers": {},
            "params": {},
...
def calculate_bezier_point(control_points, t):
...
def handle_captcha(web_page, background_ele, ym_token, retry_count):
    ...
    for i in range(int(retry_count)):
        ...
        for t in range(101):
            t /= 100
            point = calculate_bezier_point(trace_positions, t)
            path.append(point)
        print(path)
        
        for index,(offset_x, offset_y) in enumerate(path):
